...
let
	envAttrs = lib.types.attrsOf (lib.types.oneOf [ lib.types.str lib.types.int ]);
in
{
	...
	options.dotfiles.gscope = {
		defaults = lib.mkOption {
			type = envAttrs;
			default = { };
			description = "Host overrides for bin/gscope's toggles (becomes gscope/default.env).";
		};
		games = lib.mkOption {
			type = lib.types.attrsOf envAttrs;
			default = { };
			description = "Per-SteamAppId gscope overrides; win key-by-key over the global game defaults in modules/user/gaming/games.nix.";
		};
	};
	...
	config = {
		...
		# knowledge (nix/hosts/<host>/sunshine). Declaring apps here means the web
		# UI can't edit them and any local apps.json is ignored.
		services.sunshine = {
			enable = true;
